...
class SinOsc : public dsp {
  ...
  public:
	virtual void metadata(Meta* m) { 
		m->declare("name", "Sine Synthesizer");
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		float 	fSlow4 = (0.001f * float(fentry1));
		float 	fSlow5 = float(fbutton0);
		float 	fSlow6 = float(fentry2);
		float 	fSlow7 = float(fentry3);
		float 	fSlow8 = (fConst2 * (fSlow6 + fSlow7));
		float 	fSlow9 = (fConst2 * fSlow6);
		float 	fSlow10 = (fConst3 / fSlow6);
		float 	fSlow11 = (1.0f / (0 - (fConst2 * (0 - fSlow7))));
		...
		for (int i=0; i<count; i++) {
			...
			fVec1[0] = fSlow5;
			fRec3[0] = ((int((((fSlow5 - fVec1[1]) == 1) > 0)))?0:min(fSlow8, (fRec3[1] + 1)));
			int iTemp0 = int((fRec3[0] < fSlow9));
			output0[i] = (FAUSTFLOAT)((fRec0[0] * fRec2[0]) * ((iTemp0)?((int((fRec3[0] < 0)))?0:((iTemp0)?(fSlow10 * fRec3[0]):1)):((int((fRec3[0] < fSlow8)))?((fSlow11 * (fSlow9 - fRec3[0])) + 1):0)));
			// post processing
			fRec3[1] = fRec3[0];
			fVec1[1] = fVec1[0];
			fRec2[1] = fRec2[0];
