...
#ifndef FAUSTCLASS 
#define FAUSTCLASS SawOsc
#endif

class SawOsc : public dsp {
  private:
	float 	fConst0;
	float 	fConst1;
	float 	fConst2;
  ...
  public:
	virtual void metadata(Meta* m) { 
		m->declare("name", "Saw Synthesizer");
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addNumEntry("att", &fentry2, 1.0f, 1.0f, 1e+03f, 1.0f);
		ui_interface->addButton("gate", &fbutton0);
		ui_interface->addNumEntry("rel", &fentry3, 2e+02f, 1.0f, 2e+03f, 1.0f);
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		float 	fSlow0 = float(max(1e-07f, fabsf(float(fentry0))));
		float 	fSlow1 = (fConst2 * fSlow0);
		float 	fSlow2 = (1 - (fConst1 / fSlow0));
		...
		float 	fSlow9 = (fConst4 / fSlow5);
		float 	fSlow10 = (1.0f / (0 - (fConst3 * (0 - fSlow6))));
		FAUSTFLOAT* output0 = output[0];
		for (int i=0; i<count; i++) {
			float fTemp0 = (fSlow1 + (fRec0[1] + -1));
			...
			fVec0[0] = fSlow4;
			fRec3[0] = ((int((((fSlow4 - fVec0[1]) == 1) > 0)))?0:min(fSlow7, (fRec3[1] + 1)));
			int iTemp3 = int((fRec3[0] < fSlow8));
			output0[i] = (FAUSTFLOAT)((((2 * fRec1) + -1) * fRec2[0]) * ((iTemp3)?((int((fRec3[0] < 0)))?0:((iTemp3)?(fSlow9 * fRec3[0]):1)):((int((fRec3[0] < fSlow7)))?((fSlow10 * (fSlow8 - fRec3[0])) + 1):0)));
			// post processing
			fRec3[1] = fRec3[0];
			fVec0[1] = fVec0[0];
			fRec2[1] = fRec2[0];
